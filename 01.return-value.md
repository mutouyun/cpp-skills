...
<div align=center><img src="https://yuml.me/diagram/plain;dir:LR/class/[rc_string%7C-str_:char*%7C+~rc_string();+rc_string*%20clone()],[Str%7C-rc_str_:rc_string*%7C+~Str();+Str&%20operator+=(const%20char*)%7Bbg:tomato%7D],[Str]*-1%3E[rc_string],[Str]-[note:Multiple%20Str%20objects%20use%20a%20same%20rc-string%7Bbg:cornsilk%7D],[rc_base%7C+inc();+dec()]%5E-[rc_string]"/></div>
...
RVO和NRVO为我们解决了很多问题，但编译器无法应对复杂的情况。我们稍微调整一下上面的例子：
...
```
Str constructor 321
Str copy constructor 321
Str destructor 321
Str destructor 321
```

可以看出，一个简单的三目运算符就让优化失效了。另外，RVO和NRVO也不会帮我们解决资源所有权的问题，因此哪怕临时变量并不存在，拷贝构造并没有发生，下面的代码仍然无法编译通过。

```C++
Handle open_file(const std::string& name)
{
    return Handle(name); // error: 'Handle::Handle(const Handle&)' is private
}
```
